...
class GetMeituanProcessor(StatefulTextProcessor):
    ...
    def __init__(self, logger):
        ...
        self.logger.info("GetMeituanProcessor 初始化完成")

    def _reload_configs(self):
        try:
            from config.config import PROMPTS_CONFIG

            self.prompts_config = PROMPTS_CONFIG.get("get_meituan_flow", {})
            trigger_keyword = self.prompts_config.get("trigger_keyword", "获取美团")
            self.trigger_keywords = [trigger_keyword]

            self.logger.info("GetMeituanProcessor 配置重新加载成功")
    ...
    async def _ahandle_link(
        ...
        if not link_infos:
            ...
            rsp = TextRspMsg(msg)
            error_msg = self.prompts_config.get(
                "error_invalid_link",
                "❌ 未找到有效的美团短链接（http://dpurl.cn/xxx）",
            )
            continue_prompt = self.prompts_config.get(
                "continue_prompt",
    ...
    async def _aprocess_links(
        ...
        link_infos = self._extract_short_links_with_positions(text)
        self.logger.info(
            f"[{account_name}] 识别到 {len(link_infos)} 个短链接，转MP: {convert_to_mp}"
        )
        ...
        if activation_code:
            is_valid, error_msg = self.activation_manager.verify_code(
            ...
            if is_valid:
                with self.activation_manager._lock:
                    ...
                    if code_upper in code_index:
                        ...
                        self.logger.info(
                            f"[{account_name}] 激活码剩余次数: {remaining_count}"
                        )
                    else:
        ...
        async def process_single_link(idx: int, link_info: Dict[str, Any]):
            ...
            async with semaphore:
                if activation_code_info:
                    async with count_lock:
                        if remaining_count_ref[0] <= 0:
                            self.logger.warning(
                                f"[{account_name}] 激活码次数已用完，跳过第 {idx} 个链接"
                            )
                            return idx, None, "激活码次数不足"
                        remaining_count_ref[0] -= 1
                        current_remaining = remaining_count_ref[0]
                    self.logger.info(
                        f"[{account_name}] 处理第 {idx} 个链接: {short_link}，剩余次数: {current_remaining}"
                    )
                else:
                ...
                try:
                    ...
                    result_link = await api_client.agenerate_custom_link(appid, path)
                    if not result_link:
                        if activation_code_info:
                            async with count_lock:
                                remaining_count_ref[0] += 1
        ...
        if result_map and activation_code_info:
            ...
            if consumed_count > 0:
                    ...
                    try:
                        with self.activation_manager._lock:
                            ...
                            if code_upper not in code_index:
                                self.logger.warning(
                                    f"[{account_name}] 激活码已不存在，跳过保存"
                                )
                                return
                            index_entry = code_index[code_upper]
                            matched_code = index_entry["code_info"]
                            matched_user_id = index_entry["user_id"]
                            matched_code["used_count"] = matched_code.get(
                                "used_count", 0
                            ) + consumed_count
                            max_uses = matched_code.get("max_uses", 1)
                        ...
                        self.logger.info(
                            f"[{account_name}] 激活码 {activation_code} 已异步保存 {consumed_count} 次消耗（共转换 {success_count} 个链接）"
                        )
                    except Exception as e:
                        self.logger.error(
                            f"[{account_name}] 异步保存激活码消耗失败: {e}",
                            exc_info=True,
                        )
                ...
                save_thread.start()
                self.logger.info(
                    f"[{account_name}] 激活码 {activation_code} 已消耗 {consumed_count} 次使用次数（共转换 {success_count} 个链接），正在异步保存"
                )
    ...
    async def _aconvert_to_intermediate_link(self, short_link: str) -> Optional[str]:
        try:
            ...
            if not self._is_valid_meituan_redirect_host(host):
                ...
                return None
            self.logger.info(
                f"转换成功: {short_link} -> {intermediate_url}"
            )
