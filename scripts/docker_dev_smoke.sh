...
SHORTLINK_SMOKE_PAYLOAD="$(docker compose -f docker-compose.dev.yml exec -T -e WX_SMOKE_PUBLIC_BASE_URL="http://127.0.0.1:${DEV_PORT}" app python - <<'PY'
import asyncio
import json
import os
...
SHORTLINK_RESULT="$(curl -sS -o /dev/null -w '%{http_code} %{redirect_url}' "http://127.0.0.1:${DEV_PORT}${SHORTLINK_PATH}")"
SHORTLINK_STATUS="${SHORTLINK_RESULT%% *}"
SHORTLINK_LOCATION="${SHORTLINK_RESULT#* }"
docker compose -f docker-compose.dev.yml exec -T app python - "$SHORTLINK_KEY" <<'PY' >/dev/null 2>&1 || true
import asyncio
import sys
