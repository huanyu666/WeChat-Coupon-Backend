...
async def order_rankings_v2_preview(request: Request):
    ...
    return templates.TemplateResponse(request, "order_rankings_v2.html", {"request": request, "record_date": today})
...
async def order_rankings_v2_admin_preview(request: Request):
    ...
    if error:
        return error
    today = datetime.now(TIMEZONE).date().isoformat()
    return templates.TemplateResponse(request, "order_rankings_v2.html", {"request": request, "record_date": today, "admin_preview": True})
