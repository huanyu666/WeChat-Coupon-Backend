/**
 * Query the web cashback activity using the page's default smart-order list and classify infos[0] only.
 * Usage:
...
function normalizeBody(input) {
    ...
    if (!['cpsPromotionOrderMulti', 'cpsPromotionOrderRatio', 'cpsPromotionOrderKa', 'cpsPromotionOrderClosest'].includes(body.recallBizId)) {
        fail('request must use a supported web cashback recallBizId');
    }
    if (!Number.isInteger(Number(body.pageNo)) || Number(body.pageNo) < 1) {
        fail('pageNo must be a positive integer');
    }
