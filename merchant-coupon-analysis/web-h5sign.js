/**
 * web-h5sign.js - Node.js生成网页 H5Guard 4.3.0 mtgsig
 * 用法: node web-h5sign.js <url> <method> <body> [ua]
 *   ua 可选：完整桌面浏览器 User-Agent
 * 输出: JSON格式的mtgsig字符串（stdout）
 * 错误: 打印到stderr
 */
'use strict';
...
const [,, TARGET_URL, METHOD, BODY, UA_ARG] = process.argv;
if (!TARGET_URL || !METHOD || !BODY) {
    process.stderr.write('用法: node web-h5sign.js <url> <method> <body> [ua]\n');
    process.exit(1);
}
const DIR    = __dirname;
...
const UA = resolveUa(UA_ARG);

// ─── 异常捕获 ─────────────────────────────────────────
process.on('uncaughtException', function(e) {
    process.stderr.write('[h5sign error] ' + e.constructor.name + ': ' + e.message.slice(0, 200) + '\n');
    process.exit(98);
});
...
setTimeout(function() {
    ...
    setTimeout(function() {
        ...
        if (headerSigs.length === 0 && urls.length === 0) {
            process.stderr.write('[h5sign] 未捕获到mtgsig\n');
            process.exit(4);
        }
        ...
        if (!mtgsigStr) {
            ...
            if (idx === -1) {
                process.stderr.write('[h5sign] 签名URL中无mtgsig字段\n');
                process.exit(5);
            }
        ...
        process.stdout.write(mtgsigStr + '\n');
        process.exit(0);
    }, 1200);
}, 600);
