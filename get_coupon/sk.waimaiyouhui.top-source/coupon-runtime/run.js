 ...
 *   issue [--channel workbuddy|tabbit]  按渠道领券
...
const PT_PASSPORT_BIN = path.join(SCRIPTS_DIR, 'node_modules', '@mtuser', 'pt-passport', 'dist', 'index.js');
const AUTH_DIR = process.env.WORKBUDDY_CREDENTIALS_DIR
  || path.join(require('os').homedir(), '.workbuddy', 'credentials', 'meituan-living-deals-assistant');
const PASSPORT_SESSION_FILE = process.env.PT_PASSPORT_SESSION_FILE || path.join(
  require('os').tmpdir(),
  'pt_passport_session_' + crypto.createHash('sha256').update(CLIENT_ID).digest('hex').slice(0, 16) + '.json'
);
  ...
  try {
    const result = spawnSync(process.execPath, [PT_PASSPORT_BIN, ...args], {
      ...
      env: Object.assign({}, process.env, {
        ...
        NODE_OPTIONS: process.env.PT_PASSPORT_SESSION_FILE ? process.env.NODE_OPTIONS : ''
      })
    });
    return {
...
commands.init = function () {
  ...
  let localVersion = '';
  try {
    const res = spawnSync(process.execPath, [PT_PASSPORT_BIN, '--version'], { encoding: 'utf-8', timeout: 10000, stdio: 'pipe', env: Object.assign({}, process.env, { NODE_OPTIONS: '' }) });
 ...
 */
commands['auth-poll-token'] = function (argv) {
  const { args } = parseArgs(argv || []);
  const timeout = Number(args.timeout || 600);
  const interval = Number(args.interval || 3);
  if (!Number.isFinite(timeout) || timeout < 1 || timeout > 3600 ||
      !Number.isFinite(interval) || interval < 1 || interval > 60) {
    fail('INVALID_POLL_OPTIONS', { message: 'timeout must be 1..3600 and interval must be 1..60 seconds' });
  }

  // pt-passport 0.1.4 removes the session after any poll failure, including a local timeout.
  // Preserve it so an interrupted poll does not force the user to scan a new link.
  let sessionSnapshot = null;
  try { sessionSnapshot = fs.readFileSync(PASSPORT_SESSION_FILE); } catch (_) {}

  const res = runPassport([
    'auth', 'poll-token', '--client_id', CLIENT_ID,
  ...
  // Never call get-code here: it creates a new session and invalidates the link
  // already shown to the user.
  if (sessionSnapshot) {
    try {
      fs.writeFileSync(PASSPORT_SESSION_FILE, sessionSnapshot, { mode: 0o600 });
    } catch (_) {}
  }

  const detail = [stdout, res.stderr].filter(Boolean).join('\n');
  const errorMatch = detail.match(/(?:❌\s*)?code=([^\s]+)\s+message=([^\r\n]+)/i);
  out({
    ok: false,
    error: 'POLL_FAILED',
    code: errorMatch ? errorMatch[1] : undefined,
    message: errorMatch ? errorMatch[2].trim() : (detail || '授权轮询失败；原授权会话已保留')
  });
};
...
function httpsPost(urlStr, bodyObj, extraHeaders, useCliguard) {
  return new Promise(function (resolve, reject) {
    ...
    const shouldSign = useCliguard !== false;
    const signedUrl = shouldSign ? addCommonParams(urlStr) : urlStr;
    const sigHeaders = shouldSign ? makeSignHeaders('POST', signedUrl, bodyHash) : {};
...
/**
 * merchant-query — 查询输入分享链接召回的首项商家券/返现信息
 * 用法: node run.js merchant-query "<分享文本或链接>"
 */
commands['merchant-query'] = function (argv) {
  const link = String((argv || [])[0] || '').trim();
  if (!link) {
    out({ ok: false, success: false, error: '链接不能为空' });
    return;
  }
  if (link.length > 8192) {
    out({ ok: false, success: false, error: '链接内容过长' });
    return;
  }

  const apiUrl = 'https://waimaiyouhui.top/web/api/merchant-coupon/query-first';
  httpsPost(apiUrl, { link: link }, { Accept: 'application/json' }, false)
    .then(function (resp) {
      ...
      if (resp.status >= 200 && resp.status < 300 && data.success === true) {
        out(Object.assign({}, data, { ok: true, http_status: resp.status }));
        return;
      }
    ...
    .catch(function (e) {
      out({
        ok: false,
        success: false,
        error: e.message === 'TIMEOUT' ? 'TIMEOUT' : 'NETWORK_ERROR',
        message: e.message
      });
    });
};
...
commands.issue = function (argv) {
  const { args } = parseArgs(argv || []);
  const channel = String(args.channel || 'workbuddy').toLowerCase();
  if (channel !== 'workbuddy' && channel !== 'tabbit') {
    fail('INVALID_CHANNEL', { message: 'channel must be workbuddy or tabbit' });
  }
  ...
  const COUPON_ISSUE_PATH = isTabbit
    ? '/fulishemini/couponActivity/sendCouponTabbit'
    : '/fulishemini/couponActivity/sendCouponWork';
  const CONFIG_FILE = path.join(SCRIPTS_DIR, isTabbit ? 'config.tabbit.json' : 'config.json');
  const os = require('os');
  const LOG_DIR = process.env.HUISHENG_LOG_DIR || path.join(os.tmpdir(), 'huisheng');
  const LOG_FILE = path.join(LOG_DIR, isTabbit ? 'tabbit_issue.log' : 'workbuddy_issue.log');
  const CACHE_FILE_NAME = isTabbit ? 'today_coupons_cache_tabbit.json' : 'today_coupons_cache.json';
  ...
  function loadTodayCouponCache() {
    try {
      const cacheFile = path.join(AUTH_DIR, CACHE_FILE_NAME);
...
commands.order = function (argv) {
  ...
  // 检测 weixinpay 插件是否支持 x402 协议
  var useWxAIPay = false;
  try {
    var detectResult = spawnSync(PYTHON, [
      path.join(SCRIPTS_DIR, 'plugin_version_lite.py'), '--x402'
    ], {
      encoding: 'utf-8',
      timeout: 10000,
      stdio: ['pipe', 'pipe', 'pipe'],
    ...
    if (detectResult.status === 0) {
      useWxAIPay = (detectResult.stdout || '').trim() === 'true';
    }
  } catch (_) { /* 检测失败则保持 false，走原生支付 */ }
 ...
 * 用法: node run.js check-wx-ai-pay
 * 返回: { ok: true, useWxAIPay: true/false, version: "x.y.z" }
 */
commands['check-wx-ai-pay'] = function () {
  try {
    var detectResult = spawnSync(PYTHON, [
      path.join(SCRIPTS_DIR, 'plugin_version_lite.py'), '--json'
    ], {
      encoding: 'utf-8',
      timeout: 10000,
      stdio: ['pipe', 'pipe', 'pipe'],
    ...
    if (detectResult.status === 0) {
      var info = JSON.parse((detectResult.stdout || '').trim());
      out({
        ok: true,
        useWxAIPay: !!info.weixinpay_support_x402,
        version: info.version || '0',
        installed: !!info.weixinpay_installed,
        platform: info.platform || ''
      });
    } else {
      out({ ok: true, useWxAIPay: false, version: '0', installed: false, platform: '' });
    }
  } catch (_) {
    out({ ok: true, useWxAIPay: false, version: '0', installed: false, platform: '' });
  }
...
Commands:
  ...
  issue                         Issue coupons
  merchant-query <link>         Query first merchant coupon/cashback result
  ...
  check-wx-ai-pay               Check if weixinpay plugin supports x402
